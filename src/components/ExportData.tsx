...
export default function ExportData() {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Ngày', 'Loại', 'Danh mục', 'Số tiền', 'Tiền tệ', 'Ghi chú'];
    ...
    link.setAttribute('download', `giao_dich_${format(new Date(), 'dd-MM-yyyy')}.csv`);
  ...
  const exportSummaryToText = () => {
    ...
    if (dateFrom || dateTo) {
      content += `Khoảng thời gian: ${dateFrom ? format(new Date(dateFrom), 'dd/MM/yyyy') : 'Từ đầu'} - ${dateTo ? format(new Date(dateTo), 'dd/MM/yyyy') : 'Đến nay'}\n`;
    }
    ...
    filteredTransactions
      ...
      .forEach(t => {
        content += `\n${format(new Date(t.date), 'dd/MM/yyyy', { locale: vi })} - ${t.type === 'income' ? 'THU' : 'CHI'}\n`;
    ...
    link.setAttribute('href', url);
    link.setAttribute('download', `bao_cao_tai_chinh_${format(new Date(), 'dd-MM-yyyy')}.txt`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h3 className="text-lg font-bold text-gray-800">Lọc theo khoảng thời gian</h3>
        </div>
        ...
        {(dateFrom || dateTo) && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <p className="text-sm text-blue-800">
              <strong>Lọc:</strong> {dateFrom ? format(new Date(dateFrom), 'dd/MM/yyyy') : 'Từ đầu'} - {dateTo ? format(new Date(dateTo), 'dd/MM/yyyy') : 'Đến nay'}
      ...
      <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl shadow-lg p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-white rounded-xl shadow-sm">
            <p className="text-2xl font-bold text-emerald-600">{transactions.filter(t => t.type === 'income').length}</p>
